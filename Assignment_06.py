...
# JVelazquez,11.19.20,Opened Script in PyCharm
# JVelazquez,11.20.20,Modified Script to function
# JVelazquez,11.21.20,Solidified SOC + Added doc strings
# ---------------------------------------------------------------------------- #
...
class Processor:
    ...
    def add_data_to_list(task, priority, list_of_rows):
        """ Adds a row of data to Task List data table

        :param task: (string) Name of a task:
        :param priority: (string) Priority assigned to task (high/med/low):
    ...
    def remove_data_from_list(task, list_of_rows):
        """ Removes a row of data from the Task list data table
        ...
        for row in list_of_rows:
            if task in row["Task"]:
                ...
                count += 1
        if count == 0:
            IO.print_user_message("Task not found in list, nothing to remove.")
        return list_of_rows, 'Success'
...
class IO:
    ...
    def print_user_message(optional_message=''):
        ...
        :param optional_message: Message to be displayed to user
        :return:
        """
    ...
    @staticmethod
    def input_new_task_and_priority(optional_message=''):
    ...
    @staticmethod
    def input_task_to_remove(optional_message=''):
...
while(True):
    ...
    if strChoice.strip() == '1':  # Add a new Task
        ...
        if strChoice.lower() == "y":
            Processor.write_data_to_file(objFile, lstTable)
            IO.print_user_message("File Saved!")
            IO.input_press_to_continue(strStatus)
        else:
        ...
        IO.print_user_message("Warning: Unsaved data will be lost!")
        strChoice = IO.input_yes_no_choice("Are you sure you want to reload data from file? (y/n) -  ")
        if strChoice.lower() == 'y':
            Processor.read_data_from_file(strFileName, lstTable)
            IO.print_user_message("File reloaded. Data restored to last saved version.")
            IO.input_press_to_continue(strStatus)
        else:
        ...
        IO.print_user_message("Hasta Luego!")
        break   # and Exit
